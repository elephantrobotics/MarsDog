...
- 按 `VIS / AUD / TAC / NEED / EMO / BEH / EXEC / RESULT / SYS` 过滤。
- 搜索、暂停、清空和自动滚动。
...
排泄行为 `barkShortAlert` 可在 Goal 的 `params_json` 中传入布尔字段
`toilet_spot_taught`。值为 `true` 时准备阶段前往如厕垫，值为 `false`、缺失或
类型无效时原地嗅闻转圈。该参数只影响虚拟执行计划；真实执行链路仍以 Action
Feedback 的 `current_action` 为准。
...
- `触觉`：本地模拟 16 类身体触摸事件，展示情绪增量并用现有动作合同播放近似反应；不会发布 ROS Topic，也不会修改真实情绪状态。
- `视觉`：配置视觉事件、身份、姿态和物体标签。
- `行为结果`：配置 `ACTION_*`、需求类型、结果类型和 metadata。

表单会显示发布 Topic 和 JSON Payload 预览。发送前可确认实际字段，不会修改 Topic 名称或消息结构。

触觉预览使用 `local://tactile` 标识本地来源。触觉事件直接进入 Arcade 主线程，日志来源为 `TAC`；其动作由 `LocalVirtualRunner` 自测链路驱动，不创建 Action Server、不向 ROS 发布 Action Result，也不代表真实行为树决策。
...
排泄 `barkShortAlert`、浅度困倦 `sleepOnSide`、深度困倦 `sleepNow`、清洁
`lickPaws`、普通低能量 `restInPlace`、严重低能量 `recharge`、社交
`seekInteraction`、探索 `exploreRoom`。之后收到真实 Goal
...
先点击“等级 L0”按钮循环选择 L0～L3，再点击“异常模拟”。UI 会保存当前行为
Goal、Stage、动作图片、移动目标和插值进度，并按等级依次播放本地动作步骤：

- L0：紧急停止、安全姿态、查看异常位置、连续吠叫；
- L1：紧急停止、身体缩紧、回头、后退、耳朵向后、警觉；
- L2：停止、查看原因、换方向重试、短促呜咽，最后根据人物是否在场看向主人或四周；
- L3：短暂停顿、歪头查看、重新尝试。

当前行为卡会显示表格对应的本地情绪影响；L3 的焦虑保持 `+5~+10` 区间展示，
不会修改真实情绪节点。每步默认展示 0.8 秒，最后一步保持到点击“解除异常”。
本地执行计划只暂停计时，不会被 cancel；解除后会从原进度继续执行，异常持续
时间不会消耗行为的剩余时长。饥饿等待、睡眠等内部需求流程同样保留。
...
在左侧“事件”页选择“外部损伤”，可选择 12 类损伤事件、输入风险值并标记故障
是否已经确认。该入口使用 `local://external-damage`，只进入 Arcade 主线程，不发布
ROS Topic、Action 或 Service，也不代替真实传感器完成碰撞或故障诊断。

风险判级使用独立内部代码，避免与异常模拟的 L0～L3 方向混淆：

- `D1`（界面显示“损伤 L1”）：风险值大于 90，或故障已经确认；
- `D2`（界面显示“损伤 L2”）：风险值大于 70 且不超过 90；
- `D3`（界面显示“损伤 L3”）：风险值大于 40 且不超过 70；
- `NONE`：风险值不超过 40，只记录本地事件，不暂停当前行为。

超过处置阈值时，UI 会复用异常模拟的暂停锁，按所选损伤事件播放独立处置步骤。
“持续保持”和“持续至检查完成”停留在最后一步，需点击“解除损伤”；带时间范围的
处置采用每步 0.8 秒的加速演示，最后一步显示完成后自动恢复此前暂停的行为。行为卡
显示风险等级、风险分数、表格建议时长和当前 ACT，原始时长范围只作说明。
