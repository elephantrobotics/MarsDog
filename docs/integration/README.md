...
- 视觉订阅 `/emotion/state` 细分陌生人 Alert/Friend 事件，状态无效时回退通用 Stranger；不订阅内部需求状态。
- 行为树项目消费 Vision 已细分的陌生人事件，并负责事件映射、优先级、延迟
  队列、抢占、超时和会话跟踪模式。
...
- 寻物数据流由动作系统按 session 租用；人物可见时 Vision 另以可让路的2 Hz内部流
  判断手持玩具/狗粮姿态，但仍不参与搜索、接近或停车决策。
